...
export default {
  ...
  render(renderer, data) {
    ...
    renderer.appendDummyInput('_blocksTitle').appendField('对象')
    ...
    function draw(init = false) {
      ...
      if (count === 0) {
        if (renderer.getInput('_blocksTitle')) {
          renderer.removeInput('_blocksTitle')
        }
        renderer.appendDummyInput('_blocksTitle').appendField('空对象')
